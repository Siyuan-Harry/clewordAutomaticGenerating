...
def generate_cleword(content):
    ...
    user_message = f"""
              ...
              ```cleword
                ...
                - 发言:
                    ...
                    说:
                      ...
                      - |  #cleword语法注释：在“ - | ”的下面，一个缩进后，可以在同一个对话消息内写符合markdown语法的内容。比如如下所示，段落和无序列表的混排。
                        这对于更复杂的程序而言，肯定是不够用的。 
                ...
                - 发言:
                    ...
                    说:
                      - |
                        ...
                        > 只要你用心去做！
                      - | #cleword语法注释：在“ - | ”的下面，可以在同一个消息内写符合markdown语法的内容。比如这条消息中，段落 和 Python代码块的混排。
                        多说无益，先来尝试运行一下下面这个代码：
              ...
              请你注意原始文稿里的markdown语法格式（如人物标记、图片、旁白标记、无序/有序列表、代码块等），并将原始文稿逐句地转化为符合cleword语法的格式。
